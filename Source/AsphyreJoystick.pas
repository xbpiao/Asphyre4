...
//---------------------------------------------------------------------------
// AsphyreJoystick.pas                                  Modified: 28-Jan-2007
// Joystick/Gamepad DirectInput wrapper for Asphyre              Version 1.04
//---------------------------------------------------------------------------
// Important Notice:
//
...
begin
 ...
 EnumRef.Success:= Joystick.Initialize(ddi,
  TAsphyreInput(EnumRef.Referrer.FOwnerInput).WindowHandle);
...
begin
 ...
 FOwner:= AOwner;
 Assert(FOwner <> nil, msgNoOwnerSpecified);
...
var
 ...
 Flags  : Cardinal;
begin
 // (1) Retreive a valid reference to TAsphyreInput from the owner.
 Input:= TAsphyreInput(FOwner.OwnerInput);

 // (2) Create input device.
 Result:= Succeeded(Input.DirectInput8.CreateDevice(ddi.guidInstance,
  FInputDevice, nil));
...
//---------------------------------------------------------------------------
constructor TAsphyreJoysticks.Create(AOwnerInput: TObject);
begin
 inherited Create();
...
begin
 ...
 EnumRef.Referrer:= Self;
 EnumRef.Success := False;
 Result:= Succeeded(Input.DirectInput8.EnumDevices(DI8DEVCLASS_GAMECTRL,
