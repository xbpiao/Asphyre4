unit AsphyreData;
//---------------------------------------------------------------------------
// AsphyreData.pas                                      Modified: 12-Jul-2005
// Data Compression and CRC32 routines                           Version 1.01
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
uses
 Types, Classes, AsphyreZLib;

//---------------------------------------------------------------------------
type
 TCompressionLevel = (clLowest, clNormal, clHighest);

//---------------------------------------------------------------------------
// DataChecksum()
//
// Calculates a CRC32 checksum of the specified data.
// Originally written by Earl F. Glynn, 1999 (?)
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
function CompressData(Source, Dest: Pointer; SourceSize,
 MaxDestSize: Longword; Compression: TCompressionLevel): Integer;
var
 OutBuf  : Pointer;
 DestSize: Integer;
...
begin
 case Compression of
  clLowest: zLevel:= zcFastest;
  clNormal: zLevel:= zcDefault;
  else zLevel:= zcMax;
 end;
 ...
 DestSize:= 0;
 try
  ZCompress(Source, SourceSize, OutBuf, DestSize, zLevel);
 except
  Result:= 0;
  Exit;
 end;
 if (OutBuf = nil) then
  ...
  begin
   if (Int64(DestSize) <= MaxDestSize) then
    Move(OutBuf^, Dest^, DestSize)
     else Move(OutBuf^, Dest^, MaxDestSize);
...
begin
 ...
 DestSize:= 0;
 try
  ZDecompress(Source, SourceSize, OutBuf, DestSize);
 except
  Result:= 0;
  Exit;
 end;
