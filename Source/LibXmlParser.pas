...
type
  ...
  protected
    ...
    FCurEncoding: AnsiString;
    // Current Encoding during parsing (always uppercase)

    procedure AnalyzeProlog;
    // Analyze XML Prolog or Text Declaration
    procedure AnalyzeComment(Start: PAnsiChar; var Final: PAnsiChar);
    // Analyze Comments
    procedure AnalyzePI(Start: PAnsiChar; var Final: PAnsiChar);
    // Analyze Processing Instructions (PI)
    procedure AnalyzeDtdc;
    // Analyze Document Type Declaration
    procedure AnalyzeDtdElements(Start: PAnsiChar; var Final: PAnsiChar);
    ...
    procedure AnalyzeElementDecl(Start: PAnsiChar; var sFinal: PAnsiChar);
    procedure AnalyzeAttListDecl(Start: PAnsiChar; var sFinal: PAnsiChar);
    ...
    procedure ReplaceGeneralEntities(var Str: AnsiString);

    function GetDocBuffer: PAnsiChar;
    // Returns FBuffer or a pointer to a NUL char if Buffer is empty

  public // --- Document Properties
    property XmlVersion: AnsiString Read FXmlVersion;
    ...
    property RootName: AnsiString Read FRootName;
    ...
    property DocBuffer: PAnsiChar Read GetDocBuffer;
  ...
  public
    ...
    // --- Document Handling
    function LoadFromFile(Filename: AnsiString;
      FileMode: integer = fmOpenRead or fmShareDenyNone): boolean;
    // Loads Document from given file
    function LoadFromBuffer(Buffer: PAnsiChar): boolean;
    // Loads Document from another buffer
    procedure SetBuffer(Buffer: PAnsiChar);
  ...
  public
    ...
    property CurEncoding: AnsiString Read FCurEncoding; // Current Encoding
    procedure StartScan;
    function Scan: boolean;

    // --- Events / Callbacks
    function LoadExternalEntity(SystemId, PublicId, Notation: AnsiString)
      : TXmlParser; virtual;
...
const
  ...
  CLetter = [#$41 .. #$5A, #$61 .. #$7A, #$C0 .. #$D6, #$D8 .. #$F6,
    #$F8 .. #$FF];
  CDigit = [#$30 .. #$39];
  ...
  CPartType_Name: array [TPartType] of string = ('', 'XML Prolog', 'Comment',
    'PI', 'DTD Declaration', 'Start Tag', 'Empty Tag', 'End Tag', 'Text',
    'CDATA');
  CValueType_Name: array [TValueType] of string = ('Normal', 'Implied',
    'Fixed', 'Default');
  CAttrDefault_Name: array [TAttrDefault] of string = ('Default', 'Required',
    'Implied', 'Fixed');
  CElemType_Name: array [TElemType] of string = ('Empty', 'Any', 'Childs only',
    'Mixed');
  CAttrType_Name: array [TAttrType] of string = ('Unknown', 'CDATA', 'ID',
...
function Utf8ToAnsi(Source: AnsiString;
  UnknownChar: AnsiChar = '?'): AnsiString;
// Convert UTF-8 to Win-1252
...
type
  ...
  TXmlPrologEvent = procedure(Sender: TObject;
    XmlVersion, Encoding: AnsiString; Standalone: boolean) of object;
  TCommentEvent = procedure(Sender: TObject; Comment: AnsiString) of object;
  TPIEvent = procedure(Sender: TObject; Target, Content: AnsiString;
    Attributes: TAttrList) of object;
  TDtdEvent = procedure(Sender: TObject; RootElementName: AnsiString) of object;
  TStartTagEvent = procedure(Sender: TObject; TagName: AnsiString;
    Attributes: TAttrList) of object;
  TEndTagEvent = procedure(Sender: TObject; TagName: AnsiString) of object;
  TContentEvent = procedure(Sender: TObject; Content: AnsiString) of object;
  ...
  TNotationEvent = procedure(Sender: TObject;
    NotationDef: TNotationDef) of object;
  TErrorEvent = procedure(Sender: TObject; ErrorPos: PAnsiChar) of object;
  TExternalEvent = procedure(Sender: TObject; SystemId, PublicId,
    NotationId: AnsiString; var Result: TXmlParser) of object;
  TEncodingEvent = function(Sender: TObject;
    CurrentEncoding, Source: AnsiString): AnsiString of object;
  ...
  protected
    ...
    procedure WhenXmlProlog(XmlVersion, Encoding: AnsiString;
      Standalone: boolean); virtual;
    procedure WhenComment(Comment: AnsiString); virtual;
    procedure WhenPI(Target, Content: AnsiString; Attributes: TAttrList);
      virtual;
    procedure WhenDtdRead(RootElementName: AnsiString); virtual;
    procedure WhenStartTag(TagName: AnsiString; Attributes: TAttrList); virtual;
    procedure WhenEmptyTag(TagName: AnsiString; Attributes: TAttrList); virtual;
    procedure WhenEndTag(TagName: AnsiString); virtual;
    procedure WhenContent(Content: AnsiString); virtual;
    procedure WhenCData(Content: AnsiString); virtual;
  ...
  public
    ...
    procedure LoadFromBuffer(Buffer: PAnsiChar);
  ...
  protected
    ...
    property OnLoadExternal
      : TExternalEvent Read FOnLoadExternal Write FOnLoadExternal;
    property OnTranslateEncoding
      : TEncodingEvent Read FOnTranslateEncoding Write
      FOnTranslateEncoding;
  end;
...
const
  ...
  WIN1252_UNICODE: array [$00 .. $FF] of word = ($0000, $0001, $0002, $0003,
    $0004, $0005, $0006, $0007, $0008, $0009, $000A, $000B, $000C, $000D,
    $000E, $000F, $0010, $0011, $0012, $0013, $0014, $0015, $0016, $0017,
    $0018, $0019, $001A, $001B, $001C, $001D, $001E, $001F, $0020, $0021,
    $0022, $0023, $0024, $0025, $0026, $0027, $0028, $0029, $002A, $002B,
    $002C, $002D, $002E, $002F, $0030, $0031, $0032, $0033, $0034, $0035,
    $0036, $0037, $0038, $0039, $003A, $003B, $003C, $003D, $003E, $003F,
    $0040, $0041, $0042, $0043, $0044, $0045, $0046, $0047, $0048, $0049,
    $004A, $004B, $004C, $004D, $004E, $004F, $0050, $0051, $0052, $0053,
    $0054, $0055, $0056, $0057, $0058, $0059, $005A, $005B, $005C, $005D,
    $005E, $005F, $0060, $0061, $0062, $0063, $0064, $0065, $0066, $0067,
    $0068, $0069, $006A, $006B, $006C, $006D, $006E, $006F, $0070, $0071,
    $0072, $0073, $0074, $0075, $0076, $0077, $0078, $0079, $007A, $007B,
    $007C, $007D, $007E, $007F,
    ...
    case U of
      $0000 .. $007F:
        begin
          Inc(Len);
          // Result[Len] := CHR(U);
          Result[Len] := AnsiChar(CHR(U));
        end;
      $0080 .. $07FF:
        begin
          Inc(Len);
          Result[Len] := AnsiChar(CHR($C0 or (U shr 6)));
          Inc(Len);
          Result[Len] := AnsiChar(CHR($80 or (U and $3F)));
        end;
      $0800 .. $FFFF:
        begin
          Inc(Len);
          Result[Len] := AnsiChar(CHR($E0 or (U shr 12)));
          Inc(Len);
          Result[Len] := AnsiChar(CHR($80 or ((U shr 6) and $3F)));
          Inc(Len);
          Result[Len] := AnsiChar(CHR($80 or (U and $3F)));
        end;
    end;
...
begin
  ...
  for I := Length(Result) downto 1 do
    if (Result[I] in CWhitespace) then
      if PackWs and (I > 1) and (Result[I - 1] in CWhitespace) then
        Delete(Result, I, 1)
      else
        Result[I] := #32;
end;
...
function StrScanE(const Source: PAnsiChar;
  const CharToScanFor: AnsiChar): PAnsiChar;
...
procedure ExtractName(Start: PAnsiChar; Terminators: TCharset;
  var Final: PAnsiChar);
...
procedure ExtractQuote(Start: PAnsiChar; var Content: AnsiString;
  var Final: PAnsiChar);
...
function TXmlParser.LoadFromFile(Filename: AnsiString;
  FileMode: integer = fmOpenRead or fmShareDenyNone): boolean;
// Loads Document from given file
...
begin
  ...
  try
    ...
    try
      ...
      // --- Read File
      try
        BlockRead(f, FBuffer^, FBufferSize, ReadIn); (FBuffer + ReadIn)
        ^ := #0; // NULL termination
      except
        Clear;
        EXIT;
    ...
    case CurFinal^ of
      ...
      begin
        case Phase of
          ...
          phDtd:
            if (StrLComp(CurFinal, 'SYSTEM', 6) = 0) or (StrLComp(CurFinal,
                'PUBLIC', 6) = 0) then
            begin
              ExternalID := TExternalID.Create(CurFinal);
              ExternalDTD := LoadExternalEntity(ExternalID.SystemId,
                ExternalID.PublicId, '');
    ...
    begin
      ...
      begin
        if UpCase((CurFinal + 2)^) = 'X' // !!! Can't use "CHR" for Unicode characters > 255:
          then
          CurContent := CurContent + AnsiString(CHR(StrToIntDef('$' + Copy(Name, 3, MaxInt), 32)))
        else
          CurContent := CurContent + AnsiString(CHR(StrToIntDef(Copy(Name, 2, MaxInt),
              32)));
        CurFinal := P + 1;
        EXIT;
      end
      ...
      begin
        ...
        begin
          ExternalEntity := LoadExternalEntity(EntityDef.SystemId,
            EntityDef.PublicId, EntityDef.NotationName);
...
begin
  ...
  repeat
    case CurFinal^ of
      '&':
        begin
          CurContent := CurContent + TranslateEncoding
            (StrLPas(CurFinal - C, C));
          C := 0;
          ProcessEntity;
          CONTINUE;
        end;
  ...
  begin
    ...
    IsDone := CurContent <> '';
    // IsDone will only get FALSE if PackSpaces is TRUE
  end;
end;

procedure TXmlParser.AnalyzeElementDecl(Start: PAnsiChar; var sFinal: PAnsiChar);
...
begin
  ...
  repeat
    if sFinal^ = '>' then
      BREAK;
    if (sFinal^ in CNameStart) and (Element.Name = '') then
    begin
      ExtractName(sFinal, CWhitespace, f);
      SetStringSF(Element.Name, sFinal, f);
      sFinal := f;
      f := StrScan(sFinal + 1, '>');
      if f = nil then
      begin
        Element.Definition := AnsiString(sFinal);
        sFinal := StrEnd(sFinal);
        BREAK;
      end
      else
      begin
        SetStringSF(Element.Definition, sFinal + 1, f - 1);
        sFinal := f;
        BREAK;
      end;
    end;
...
procedure TXmlParser.AnalyzeAttListDecl(Start: PAnsiChar; var sFinal: PAnsiChar);
...
var
  ...
  DER: TDtdElementRec;
begin
  sFinal := Start + 9; // The character after <!ATTLIST
  Phase := phElementName;
  DER.Start := Start;
  AttrDef := nil;
  ElemDef := nil;
  repeat
    if not(sFinal^ in CWhitespace) then
      case sFinal^ of
        '%':
          begin
            PushPE(sFinal);
        ...
        case Phase of
          phElementName:
            begin
              ExtractName(sFinal, CWhitespace + CQuoteChar + ['#'], f);
              SetStringSF(ElementName, sFinal, f);
              sFinal := f;
          ...
          phName:
            begin
              ...
              ExtractName(sFinal, CWhitespace + CQuoteChar + ['#'], f);
              SetStringSF(AttrDef.Name, sFinal, f);
              sFinal := f;
          ...
          phType:
            begin
              ...
              begin
                f := StrScan(sFinal + 1, ')');
                if f <> nil then
                  SetStringSF(AttrDef.TypeDef, sFinal + 1, f - 1)
                else
                  AttrDef.TypeDef := AnsiString(sFinal + 1);
              ...
              else if StrLComp(sFinal, 'NOTATION', 8) = 0 then
              begin
                Inc(sFinal, 8);
              ...
              begin
                ExtractName(sFinal, CWhitespace + CQuoteChar + ['#'], f);
                SetStringSF(AttrDef.TypeDef, sFinal, f);
          ...
          phNotationContent:
            begin
              f := StrScan(sFinal, ')');
              if f <> nil then
                SetStringSF(AttrDef.Notations, sFinal + 1, f - 1)
              else
              begin
                AttrDef.Notations := AnsiString(sFinal + 1);
                sFinal := StrEnd(sFinal);
              end;
          ...
          phDefault:
            begin
              ...
              else if (sFinal^ in CQuoteChar) then
              begin
                ExtractQuote(sFinal, AttrDef.Value, sFinal);
...
begin
  ...
  begin
    ...
    ReplaceParameterEntities(EntityDef.Value);
    // Create replacement texts (see XmlSpec 4.5)
    ReplaceCharacterEntities(EntityDef.Value);
  end;
...
procedure TXmlParser.AnalyzeNotationDecl(Start: PAnsiChar;
  var Final: PAnsiChar);
...
begin
  ...
  repeat
    ...
    if CompareText(Str[PosAmp + 2], 'x') = 0 // !!! Can't use "CHR" for Unicode characters > 255
      then
      Str[PosAmp] := AnsiChar(CHR(StrToIntDef('$' + Copy(Str, PosAmp + 3, Len - 4), 0)))
    else
      Str[PosAmp] := AnsiChar(CHR(StrToIntDef(Copy(Str, PosAmp + 2, Len - 3), 32)));
...
procedure TXmlParser.ReplaceGeneralEntities(var Str: AnsiString);
      ...
      begin
        ...
        begin
          ...
          begin // External Entity
            ExternalEntity := LoadExternalEntity(EntityDef.SystemId,
              EntityDef.PublicId, EntityDef.NotationName);
            Repl := StrPas(ExternalEntity.DocBuffer);
            // !!! What if it contains a Text Declaration?
            ExternalEntity.Free;
          end;
          ReplaceEntities(Repl); // Recursion
        end
...
function TXmlParser.LoadExternalEntity(SystemId, PublicId, Notation: AnsiString)
  : TXmlParser;
...
function TScannerXmlParser.LoadExternalEntity(SystemId, PublicId,
  Notation: AnsiString): TXmlParser;
begin
...
function TScannerXmlParser.TranslateEncoding(const Source: AnsiString)
  : AnsiString;
begin
...
procedure TCustomXmlScanner.WhenXmlProlog(XmlVersion, Encoding: AnsiString;
  Standalone: boolean);
...
procedure TCustomXmlScanner.WhenComment(Comment: AnsiString);
...
procedure TCustomXmlScanner.WhenPI(Target, Content: AnsiString;
...
procedure TCustomXmlScanner.WhenDtdRead(RootElementName: AnsiString);
...
procedure TCustomXmlScanner.WhenStartTag(TagName: AnsiString;
  Attributes: TAttrList);
...
procedure TCustomXmlScanner.WhenEmptyTag(TagName: AnsiString;
  Attributes: TAttrList);
...
procedure TCustomXmlScanner.WhenEndTag(TagName: AnsiString);
...
procedure TCustomXmlScanner.WhenContent(Content: AnsiString);
...
procedure TCustomXmlScanner.WhenCData(Content: AnsiString);
  ...
  while FXmlParser.Scan and (not FStopParser) do
    case FXmlParser.CurPartType of
      ptNone:
        ;
      ptXmlProlog:
        WhenXmlProlog(FXmlParser.XmlVersion, FXmlParser.Encoding,
          FXmlParser.Standalone);
      ptComment:
